...
// 素材管理（仅管理员）：create / toggleActive / delete
exports.main = async (event) => {
  ...
  try {
    if (action === 'create') {
      const res = await db.collection('res_materials').add({
        data: Object.assign({ isActive: true, sort: 999, createTime: Date.now() }, data),
      })
      return { success: true, _id: res._id }
    }
    if (action === 'toggleActive') {
      await db.collection('res_materials').doc(id).update({ data: { isActive } })
      return { success: true }
    }
    if (action === 'delete') {
      await db.collection('res_materials').doc(id).remove()
      return { success: true }
    }
